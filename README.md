...
The final Colab notebook used a compact convolutional U-Net implemented directly in PyTorch. Training and validation loss curves and prediction visualizations were used for qualitative analysis alongside the reported Mean IoU and Dice Score metrics.
...
The final Colab run used the following architecture comparison: Baseline Encoder-Decoder vs U-Net, with both trained under the same segmentation setup and evaluated using Mean IoU and Dice Score. The U-Net showed a clear improvement over the baseline, while the Dice + Cross-Entropy variant provided a modest additional gain. The final results are: U-Net vs Baseline Mean IoU improvement = 45.73%, U-Net vs Baseline Dice Score improvement = 31.43%, Dice + Cross-Entropy vs Cross-Entropy Mean IoU improvement = 0.40%, Dice + Cross-Entropy vs Cross-Entropy Dice Score improvement = 0.40%.

For the loss comparison, the absolute differences were: Mean IoU = +0.0026 and Dice Score = +0.0031.
